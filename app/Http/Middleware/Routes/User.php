...
class User
{
    ...
    {
        if ($request->is('api/v1/en/users') || $request->is('api/v1/users')) {
            if ($request->isMethod('GET')) {
                if (!auth()->user()->can('retorna usuários')) {
                ...
                if (!auth()->user()->can('insere usuário')) {
                    $errors = $this->respondWithError($request->url());
        ...
        if ($request->is('api/v1/en/users/*') || $request->is('api/v1/users/*')) {
            if ($request->isMethod('GET')) {
                if (!auth()->user()->can('retorna usuário')) {
                    $errors = $this->respondWithError($request->url());
            ...
            if ($request->isMethod('PUT')) {
                if (!auth()->user()->can('altera usuário')) {
                    $errors = $this->respondWithError($request->url());
            ...
            if ($request->isMethod('DELETE')) {
                if (!auth()->user()->can('remove usuário')) {
                    $errors = $this->respondWithError($request->url());
        ...
        if ($request->is('api/v1/en/users/*/edit') || $request->is('api/v1/users/*/edit')) {
            if (!auth()->user()->can('altera usuário')) {
                $errors = $this->respondWithError($request->url());
